...
function valueMin() {
   ...
   if (event.target.value.replace(",", ".") <= 0.00) { // condição
      if (validationElement) { validationElement.remove() } // ação
   } else if (event.target.value.replace(",", ".") <= 0.09) { // condição
      if (!validationElement) { wrapperTroco.insertAdjacentHTML("beforeend", '<div class="validation"><span>valor mínimo: <b>R$ 0,10</b></span></div>') } // ação
   } else { // condição
      if (validationElement) { validationElement.remove() } //ação
   }
}
...
function generatorNum() {
   ...
   ContainerPrincipal.insertAdjacentHTML(
      "beforeend",
      '<div id="constrast"></div>'
   );
